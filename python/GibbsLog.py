##change on prob calculation, still using log.
    ...
    for epoch in range(epoches):
        ...
        for i in range(len(assigned_corpus)):
            ...
            ## desampling
                ...
                probability = np.log(doc_likes_topic+1)*np.log(np.transpose(topic_likes_word)+1)
...
if __name__=='__main__':
   ...
   #save_model(doc_topic,topic_word,corpus,ll_list,'GibbsModel4000')
   doc_topic += 1
   doc_topic = doc_topic/np.transpose([np.sum(doc_topic,axis = 1)])
   ...
   plt.ylabel('Perplexity (Log-Probability Sampling)')
   plt.show()
